...
class InfluxDBHandler:
    ...
    def get_data_by_batch_id(self, batch_id):
        ...
        try:
            ...
            query = f"""
            from(bucket: "{self.bucket}")
            |> range(start: -15d)  // Adjust the time range as needed
    ...
    def get_data_by_batch_id2(self, batch_id):
        ...
        try:
            ...
            |> range(start: -20d)  // Adjust the time range as needed
    ...
    def get_experiment_duration(self, experiment_ID, time_range="-30d"):
        """
        Fetch the duration of an experiment based on its ID.

        Args:
            experiment_ID (str): The ID of the experiment.
            time_range (str): The time range to query (default is "-30d").
    ...
    def get_data_for_table(self, bucket_name, experiment_ID, time_range="-30d"):
        """
    ...
    def get_data_test(self, experiment_id, selected_metric):
        try:
            if not experiment_id or not selected_metric:
                raise ValueError("El experiment_id y la métrica no pueden ser None o vacíos.")

            # Consulta Flux para obtener solo los valores de la métrica seleccionada
            query = f"""
            from(bucket: "{self.bucket}")
                |> range(start: -30d)  // Ajustar el rango de tiempo según necesidad
                |> filter(fn: (r) => r["{str(BACH_ID)}"] == "{str(experiment_id)}")
                |> filter(fn: (r) => r["_field"] == "{selected_metric}")
            ...
            print(f"Error obteniendo datos de test para experiment_id {experiment_id}: {e}")
            return []
