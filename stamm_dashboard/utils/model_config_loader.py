...
class ModelConfigLoader:
    ...
    # Función para cargar inputs desde un archivo YAML
    ...
    def get_metrics_score_options(self):
        ...
        metrics = set()  # Usamos un set para evitar valores duplicados

        for config in self.configurations_monitoring:
            try:
                # Extraer el nombre del detector de la configuración
                name = config.get('drift_detector', {}).get('acronym', None)

                if name:  # Evitar valores vacíos o None
                    metrics.add(name)

            except Exception as e:
                logging.error(f"Error processing configuration monitoring: {e}")

        # Convertimos los valores únicos en una lista de diccionarios
        return [{"label": metric, "value": metric} for metric in sorted(metrics)]
    
    #Busca la métrica seleccionada en los archivos YAML ya cargados en configurations_monitoring p
    def load_metric_descriptions(self, selected_metric):
        # Valores por defecto en caso de no encontrar la métrica
        default_response = {
            "name": "Unknown Metric",
        ...
        for config in self.configurations_monitoring:
            try:
                ...
                print(f"Error al obtener la información de la métrica {selected_metric}: {e}")

        return default_response
    #Obtiene todas las categorías únicas del campo 'type' dentro de los inputs.
        ...
        for config in self.configurations:
            try:
                if model_name:
                    ...
                    print("Modelo no encontrado en la configuración")
                    return []  # O devuelve una lista vacía []
            except Exception as e:
        ...
        for config in self.configurations:
            try:
                if category:
                    ...
                    print("Modelo no encontrado name input")
                    return []  # O devuelve una lista vacía []
            except Exception as e:
    ...
    def get_performance_estimators_options(self):
        ...
        for config in self.configurations_monitoring:
            try:
                # Extraer la lista de detectores de la configuración
                ...
                if isinstance(drift_detectors, list):
                    for detector in drift_detectors:
                        ...
                        if name and acronym:  # Asegurar que ambos valores existen
    ...
    def load_estimator_descriptions(self, selected_estimator):
        ...
        for config in self.configurations_monitoring:
            try:
                ...
                logging.error(f"Error al obtener la información de la métrica {selected_estimator}: {e}")
        
        logging.warning(f"Métrica no encontrada: {selected_estimator}. Datos disponibles: {self.configurations_monitoring}")
        return default_response
